...
export interface LocalSkillItem { id: string; name: string; description: string; origin: LocalSkillOrigin }
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const frontmatter = parseSkillMarkdownFrontmatter(await readBoundedFile(markdownPath));
        items.push({ id: frontmatter.slug ?? entry.name, name: frontmatter.name, description: frontmatter.description, origin: root.origin });
