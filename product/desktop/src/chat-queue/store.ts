...
export interface ChatQueueStore {
  ...
  referencedAttachmentIds(): Promise<ReadonlySet<string>>;
}
...
export function createChatQueueStore(dataRoot: string, options: ChatQueueStoreOptions = {}): ChatQueueStore {
  ...
  return {
    ...
    referencedAttachmentIds: () => serialize(path, async () => new Set(Object.keys((await readAuthority()).attachmentReferences))),
  };
}
