...
export function MessageContent({ blocks }: { blocks: ContentBlock[] }) {
  return <div className="message-content">{blocks.map((block) => {
    switch (block.type) {
      case "text": return block.format === "markdown" ? <SafeMarkdown key={block.id} text={block.text} /> : <p key={block.id}>{block.text}</p>;
      ...
      case "image": return block.sourceUrl === undefined
        ? <p key={block.id}>图片：{block.alt ?? block.file.name}</p>
        : <MessageImage key={block.id} sourceUrl={block.sourceUrl} alt={block.alt ?? block.file.name} suggestedName={block.file.name} />;
