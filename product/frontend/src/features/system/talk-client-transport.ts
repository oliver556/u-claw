...
export async function startTalkClientTransport(bootstrap: TalkClientBootstrap, runtime: TalkClientRuntime = browserRuntime, control?: TalkClientControl): Promise<TalkClientTransport> {
  const url = new URL(bootstrap.offerUrl);
  ...
  channel.addEventListener("message", (event) => { void (async () => {
    ...
    try {
      ...
      if (name === "openclaw_agent_consult") { activeConsults.add(callId); try { sendToolOutput(callId, await control.consult({ sessionKey: bootstrap.sessionKey, callId, name, args })); } finally { activeConsults.delete(callId); } }
      else if (name === "openclaw_agent_control") { const input = args && typeof args === "object" ? args as Record<string, unknown> : {}; const text = [input.text, input.message, input.request, input.query].find((item) => typeof item === "string" && item.trim()) as string | undefined; const mode = input.mode; if (!text) throw new Error("Talk control text is required."); if (mode !== undefined && mode !== "status" && mode !== "steer" && mode !== "cancel" && mode !== "followup") throw new Error("Invalid Talk control mode."); const result = await control.steer({ sessionKey: bootstrap.sessionKey, text: text.trim(), ...(mode ? { mode } : {}) }); const row = result && typeof result === "object" ? result as Record<string, unknown> : {}; if (responseActive && (row.mode === "cancel" || row.suppress === true && row.mode !== "steer")) channel.send(JSON.stringify({ type: "response.cancel" })); sendToolOutput(callId, result); }
  ...
  try { const offer = await peer.createOffer(); await peer.setLocalDescription(offer); const response = await runtime.fetch(bootstrap.offerUrl, { method: "POST", body: offer.sdp, headers: { ...bootstrap.offerHeaders, Authorization: `Bearer ${secret}`, "Content-Type": "application/sdp" } }); if (!response.ok) throw new Error(`Talk WebRTC setup failed (${response.status})`); await peer.setRemoteDescription({ type: "answer", sdp: await response.text() }); }
