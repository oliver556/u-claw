...
export function ReleaseCenter({ bridge }: { bridge?: ReleaseBridge }) {
  ...
  return <section className="release-center secondary-view">
    <header><div><h1>发布与恢复</h1><p>签名更新、版本回滚与受控卸载</p></div><div className="release-head-actions">{checking ? <button className="secondary-command" type="button" onClick={() => void cancelCheck()}><X />取消检查</button> : null}</div></header>
