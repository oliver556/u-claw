...
export function InstalledSkillWorkbench() {
  ...
  return <section className="skill-workbench" aria-label="本地 Skill 工作台">
    <header className="skill-workbench-head"><div><h1>Skill</h1><p>管理 U 盘中已安装的能力与 OpenClaw 运行状态</p></div><div className="skill-workbench-actions"><button type="button" aria-label="导入 Skill" disabled={importState !== "idle"} onClick={() => void beginImport()}>{importState === "selecting" || importState === "validating" ? <RefreshCw className="spin" /> : <PackageOpen />}{importState === "selecting" ? "选择中" : importState === "validating" ? "校验中" : "导入 Skill"}</button><button type="button" className="primary" aria-label="发现更多 Skill" onClick={() => void openHub()}><ExternalLink />发现更多</button></div></header>
    ...
    <div className="skill-workbench-toolbar"><label><Search /><input aria-label="搜索本地 Skill" placeholder="搜索名称、说明或来源" value={query} onChange={(event) => setQuery(event.target.value)} /></label><select aria-label="按状态筛选" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">状态：全部</option>{Object.entries(availabilityLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select aria-label="按来源筛选" value={source} onChange={(event) => setSource(event.target.value)}><option value="all">来源：全部</option><option value="openclaw">本地工作区</option><option value="portable">随包内置</option><option value="skillhub">用户导入</option></select><span className="skill-result-count">{filtered.length} 个 Skill</span><button type="button" title="刷新" aria-label="刷新本地 Skill" onClick={() => void load()}><RefreshCw /></button><button type="button" title="列表显示" aria-label="Skill 列表显示"><SlidersHorizontal /></button></div>
    {state === "loading" ? <div className="skill-workbench-loading" aria-label="正在读取本地 Skill">{[0, 1, 2].map((value) => <span key={value} />)}</div> : null}
    {state === "error" ? <div className="skill-error" role="alert"><AlertTriangle /><div><strong>本地 Skill 读取失败</strong><span>{error}</span></div><button onClick={() => void load()}>重试</button></div> : null}
    {actionError ? <div className="skill-error" role="alert"><AlertTriangle /><div><strong>Skill 操作失败</strong><span>{actionError}</span></div><button type="button" onClick={() => setActionError("")}>关闭</button></div> : null}
    ...
    {state === "ready" && items.length > 0 && filtered.length === 0 ? <div className="skill-workbench-empty"><Search /><strong>没有符合条件的 Skill</strong><button type="button" aria-label="清除筛选" onClick={() => { setQuery(""); setStatus("all"); setSource("all"); }}>清除筛选</button></div> : null}
