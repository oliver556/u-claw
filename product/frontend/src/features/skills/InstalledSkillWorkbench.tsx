...
import { AlertTriangle, Box, CircleCheck, Download, ExternalLink, Eye, Layers3, PackageOpen, RefreshCw, Search, X } from "lucide-react";
...
const riskLabel = { low: "低风险", medium: "中风险", high: "高风险", critical: "严重风险" } as const;
...
export function InstalledSkillWorkbench() {
  ...
  /** Keeps installation failures inside the confirmation surface that initiated them. */
  const [importError, setImportError] = useState("");
  /** Exposes external navigation latency and prevents duplicate hub requests. */
  ...
  /** Holds the exact installed permission snapshot that must be accepted before re-enabling. */
  ...
  /** Opens the fixed SkillHub destination with immediate feedback and a single in-flight request. */
  const openHub = async () => {
    if (openingHub) return;
    setActionError(""); setOpeningHub(true);
    try { await call("skills.open-hub", {}); }
    catch (caught) { setActionError(caught instanceof Error ? caught.message : "无法打开 SkillHub"); }
    finally { setOpeningHub(false); }
  };
  /** Selects and validates a local ZIP before exposing its risk confirmation. */
  ...
  const installImport = async () => {
    ...
    try {
      let operation = await call<SkillOperation>("skills.import-install", { token: candidate.token, confirmation: { permissionFingerprint: candidate.detail.permissionFingerprint, acceptedRisk: candidate.detail.risk } });
      ...
      setToast({ kind: "success", message: `${candidate.detail.name} 安装成功，OpenClaw 已完成读回` });
      await load();
    } catch (caught) {
      const message = caught instanceof Error ? caught.message : "Skill 安装失败";
  ...
  /** Disposes the consumed selection before reopening the native ZIP picker after a failure. */
  const restartImport = async () => {
    const candidate = importCandidate;
    setImportCandidate(undefined); setImportError(""); setConfirmed(false);
    if (candidate) await call("skills.import-dispose", { token: candidate.token }).catch(() => undefined);
    await beginImport();
  };
  /** Applies one explicit enablement target and keeps the switch locked until authoritative reload. */
  const applyToggle = async (item: SkillCatalogItem, nextEnabled: boolean) => {
    setActionError("");
    setTogglingSlug(item.slug);
    try {
      const confirmation = nextEnabled && item.permissions.length > 0
        ? { permissionFingerprint: item.permissionFingerprint, acceptedRisk: item.risk }
        : null;
      const operation = await call<SkillOperation>("skills.set-enabled", { slug: item.slug, enabled: nextEnabled, confirmation });
      if (operation.state !== "succeeded") throw new Error(operation.error ?? "OpenClaw 未确认 Skill 启用状态");
      await load();
      setEnableCandidate(undefined); setEnableConfirmed(false);
  ...
  /** Routes permission-bearing enablement through explicit confirmation; disablement stays direct. */
  const toggle = (item: SkillCatalogItem) => {
    const nextEnabled = !item.enabled;
    if (nextEnabled && item.permissions.length > 0) {
      setEnableCandidate(item); setEnableConfirmed(false); setToast(undefined);
      return;
    }
    void applyToggle(item, nextEnabled);
  };

  return <section className="skill-workbench" aria-label="本地 Skill 工作台">
    <header className="skill-workbench-head"><div><h1>我的技能</h1><p>管理已安装能力与 OpenClaw 运行状态</p></div><div className="skill-workbench-actions"><Button aria-label="导入 Skill" loading={importState === "selecting" || importState === "validating"} disabled={importState !== "idle"} icon={<PackageOpen />} onClick={() => void beginImport()}>{importState === "selecting" ? "选择中" : importState === "validating" ? "校验中" : "导入 Skill"}</Button><Button type="primary" aria-label="发现更多 Skill" loading={openingHub} disabled={openingHub} icon={<ExternalLink />} onClick={() => void openHub()}>发现更多</Button></div></header>
    ...
    <div className="skill-workbench-toolbar"><Input className="skill-workbench-search" aria-label="搜索本地 Skill" placeholder="搜索名称、说明或来源" value={query} prefix={<Search aria-hidden="true" />} allowClear onChange={(event) => setQuery(event.target.value)} /><Select className="skill-workbench-select" aria-label="按状态筛选" value={status} virtual={false} onChange={setStatus} options={[{ value: "all", label: "状态：全部" }, ...Object.entries(availabilityLabel).map(([value, label]) => ({ value, label: `状态：${label}` }))]} /><Select className="skill-workbench-select" aria-label="按来源筛选" value={source} virtual={false} onChange={setSource} options={[{ value: "all", label: "来源：全部" }, { value: "openclaw", label: "来源：本地工作区" }, { value: "portable", label: "来源：随包内置" }, { value: "skillhub", label: "来源：用户导入" }]} /><span className="skill-result-count">{filtered.length} 个 Skill</span><Button title="刷新" aria-label="刷新本地 Skill" loading={state === "loading"} disabled={state === "loading"} icon={<RefreshCw />} onClick={() => void load()} /></div>
    ...
    {enableCandidate ? <div className="skill-dialog-backdrop"><div className="skill-dialog" role="dialog" aria-modal="true" aria-label={`确认启用 ${enableCandidate.name}`}><header><AlertTriangle /><div><strong>{riskLabel[enableCandidate.risk]}权限确认</strong><span>{enableCandidate.name}</span></div></header><div className="skill-dialog-permissions">{enableCandidate.permissions.map((permission) => <div key={`${permission.kind}-${permission.target}`}><strong>{permissionKindLabel[permission.kind]} · {permission.target}</strong><span>{permission.reason}</span><em className={`risk-${permission.risk}`}>{riskLabel[permission.risk]}</em></div>)}</div><label className="skill-risk-confirm"><input type="checkbox" checked={enableConfirmed} disabled={togglingSlug === enableCandidate.slug} onChange={(event) => setEnableConfirmed(event.target.checked)} />我已了解{riskLabel[enableCandidate.risk]}权限</label><footer><Button disabled={togglingSlug === enableCandidate.slug} onClick={() => { setEnableCandidate(undefined); setEnableConfirmed(false); }}>取消</Button><Button type="primary" aria-label="确认启用" loading={togglingSlug === enableCandidate.slug} disabled={!enableConfirmed || togglingSlug === enableCandidate.slug} onClick={() => void applyToggle(enableCandidate, true)}>确认启用</Button></footer></div></div> : null}
    {importCandidate ? <div className="skill-dialog-backdrop"><div className="skill-dialog" role="dialog" aria-modal="true" aria-label={`确认导入 ${importCandidate.detail.name}`}><header><AlertTriangle /><div><strong>高风险安装确认</strong><span>{importCandidate.detail.name} · v{importCandidate.detail.version}</span></div></header><p>{importState === "installing" ? "正在安装并等待 OpenClaw 读回确认…" : "包已通过结构与路径校验。安装后将由 OpenClaw 权威读回确认。"}</p>{importError ? <Alert type="error" showIcon role="alert" message="安装失败" description={importError} /> : null}<label className="skill-risk-confirm"><input type="checkbox" checked={confirmed} disabled={importState === "installing" || Boolean(importError)} onChange={(event) => setConfirmed(event.target.checked)} />我已了解网络来源 Skill 的高风险</label><footer><Button disabled={importState === "installing"} onClick={() => void cancelImport()}>{importError ? "关闭" : "取消"}</Button>{importError ? <Button type="primary" aria-label="重新选择 Skill" onClick={() => void restartImport()}>重新选择</Button> : <Button type="primary" aria-label="确认安装" aria-busy={importState === "installing"} loading={importState === "installing"} disabled={!confirmed || importState === "installing"} onClick={() => void installImport()}>确认安装</Button>}</footer></div></div> : null}
    </div>
  </section>;
}
