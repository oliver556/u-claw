...
export function ContextPanel({ client, session, capabilities, activity, onClose, onSessionReadback }: ContextPanelProps) {
  return (
    <aside className="context-panel" aria-label="上下文舱">
      ...
      <ContextTabs client={client} session={session} capabilities={capabilities} activity={activity} onSessionReadback={onSessionReadback} />
    </aside>
  );
}
