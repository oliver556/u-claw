...
function bridge(state: "available" | "offline" | "unavailable" = "available", recovery: "clean" | "rolled-back" | "recovery-required" = "clean", operationRunning = false, operationPhase: "cleaning" | "switching" = "cleaning") {
  const invoke = vi.fn(async (request: any) => {
    ...
    if (["release.check", "release.retry"].includes(request.method)) return ok({ state, checkedAt: "2026-08-09T00:00:00.000Z", currentVersion: "0.1.0", channel: "stable", retryable: state !== "unavailable", ...(state === "available" ? { update: { id: "release-42", version: "0.2.0", channel: "stable", publishedAt: "2026-08-09T00:00:00.000Z", notes: ["安全更新", "修复恢复流程"], compatibility: { platform: "win32", arch: "x64", runtimeId: "openclaw-2026.7.1-2-win-x64" }, bytes: 128, mandatory: false, previewToken: "preview-42" } } : {}) });
