...
describe("Talk client WebRTC transport", () => {
  it("uses one-time credential, microphone track, SDP, and disposes all resources", async () => {
    ...
    const transport = await startTalkClientTransport({ provider: "openai", transport: "webrtc", clientSecret: "short-lived", offerUrl: "https://api.openai.com/v1/realtime/calls", sessionKey: "main" }, { getUserMedia: vi.fn(async () => media as never), createPeer: () => peer as never, fetch, createAudio: () => audio }); expect(peer.addTrack).toHaveBeenCalled(); expect(peer.setRemoteDescription).toHaveBeenCalledWith({ type: "answer", sdp: "answer-sdp" }); transport.stop(); expect(track.stop).toHaveBeenCalled(); expect(peer.close).toHaveBeenCalled(); expect(audio.isConnected).toBe(false);
  });
