...
export const ClientIpcRequestSchema = z.discriminatedUnion("method", [
  ...
  z.object({ method: z.literal("session-organizer.remove-group"), requestId: RequestIdSchema, params: z.object({ groupId: z.string().min(1) }).strict() }).strict(),
...
export const ClientIpcSuccessResponseSchema = z.discriminatedUnion("method", [
  ...
  z.object({ method: z.literal("session-organizer.remove-group"), requestId: RequestIdSchema, ok: z.literal(true), result: SessionOrganizerDocumentSchema }).strict(),
...
export const ClientIpcFailureResponseSchema = z
  .object({
    method: z.enum([
      ...
      "session-organizer.get", "session-organizer.set-pinned", "session-organizer.create-group", "session-organizer.rename-group", "session-organizer.remove-group", "session-organizer.assign-group",
