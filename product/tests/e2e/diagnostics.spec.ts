...
async function installDiagnosticsFixture(page: import("@playwright/test").Page) {
  await page.addInitScript(() => {
    ...
    const invoke = async (request: any) => {
      ...
      if (request.method === "network.run") return { method: request.method, requestId: request.requestId, ok: true, result: { mode: "intranet-only", checks: ["portable-data", "runtime", "gateway", "local-port", "dns", "provider", "channels", "capabilities"].map((id) => ({ id, label: id, status: id === "provider" ? "unreachable" : "passed", level: id === "provider" ? "warning" : "info", summary: id === "provider" ? "外网不可用，内网功能仍可使用。" : "检查通过。", durationMs: 10 })), proxy: { configured: true, noProxyConfigured: true } } };
