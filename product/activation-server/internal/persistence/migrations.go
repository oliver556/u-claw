...
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	...
	var checksum []byte
	err = tx.QueryRow(ctx, "SELECT checksum FROM schema_migrations WHERE version = $1", migrationVersion).Scan(&checksum)
	switch {
	case err == nil:
		if !equalBytes(checksum, initialMigrationChecksum) {
	...
	case errors.Is(err, pgx.ErrNoRows):
		...
		if _, err := tx.Exec(ctx, "INSERT INTO schema_migrations (version, checksum, applied_at) VALUES ($1, $2, now())", migrationVersion, initialMigrationChecksum); err != nil {
