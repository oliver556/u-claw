...
if [ ! -x "$APP_BIN" ] || [ "$CURRENT_STAMP" != "$CACHED_STAMP" ]; then
  ...
  if [ ! -f "$LOCAL_ARCHIVE" ] || [ "$LOCAL_STAMP" != "$CURRENT_STAMP" ]; then
    ...
    echo "[Bavi-box] Copying Mac archive to local cache..."
    rm -f "$TMP_ARCHIVE"
    ...
    while kill -0 "$COPY_PID" 2>/dev/null; do
      ...
      if kill -0 "$COPY_PID" 2>/dev/null; then
        ...
        if [ "$ARCHIVE_BYTES" -gt 0 ]; then
          ...
          echo "[Bavi-box] Copying Mac archive... ${COPIED_MB}/${ARCHIVE_MB} MB (${PERCENT}%), ${ELAPSED}s elapsed."
          if [ "$ELAPSED" -ge 60 ] && [ "$COPIED_BYTES" -eq 0 ]; then
            echo "[Bavi-box] USB read has not reported byte progress yet; continuing local archive cache copy."
          fi
        else
          echo "[Bavi-box] Copying Mac archive... ${ELAPSED}s elapsed."
        fi
      fi
      if [ "$ELAPSED" -ge "$COPY_TIMEOUT_SECONDS" ]; then
        ...
        rm -f "$TMP_ARCHIVE"
        echo "[Bavi-box] Failed to copy Mac archive within ${COPY_TIMEOUT_SECONDS}s."
        if [ -x "$APP_BIN" ]; then
          echo "[Bavi-box] Existing app cache is available; starting cached app instead."
          USE_EXISTING_APP_CACHE=1
          break
        else
          echo "[Bavi-box] Please reconnect the USB disk or copy the Bavi-box folder to a healthier disk."
          wait_before_exit
          exit 1
        fi
      fi
    ...
    if [ "$USE_EXISTING_APP_CACHE" != "1" ] && [ "$USE_DIRECT_ARCHIVE" != "1" ] && ! wait "$COPY_PID"; then
      rm -f "$TMP_ARCHIVE"
      echo "[Bavi-box] Failed to copy Mac archive."
      if [ -x "$APP_BIN" ]; then
        echo "[Bavi-box] Existing app cache is available; starting cached app instead."
        USE_EXISTING_APP_CACHE=1
      else
    ...
    if [ "$USE_EXISTING_APP_CACHE" != "1" ] && [ "$USE_DIRECT_ARCHIVE" != "1" ]; then
      ...
      echo "[Bavi-box] Mac archive copied in $(($(date +%s) - COPY_STARTED_AT))s."
    fi
  else
    echo "[Bavi-box] Reusing local Mac archive cache."
  fi
  if [ "$USE_EXISTING_APP_CACHE" != "1" ]; then
    ...
    echo "[Bavi-box] Installing updated app cache..."
    ...
    if [ ! -x "$TMP_CACHE_DIR/Bavi-box.app/Contents/MacOS/Bavi-box" ]; then
