...
- `verify-ecommerce-workbench-ui.js` runs connected browser acceptance for the ecommerce image workbench upload, platform selection, stubbed direct image API call, in-page image results, persisted records, and screenshot.
- `verify-ecommerce-electron-ipc.js` starts real Electron and verifies the ecommerce Prompt Pack preload bridge reaches the main-process IPC handler.
