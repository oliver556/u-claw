...
function buildConfig(options, sourceConfigs) {
  ...
  for (const providerName of ['custom', 'litellm']) {
    ...
    providers[providerName].baseUrl = normalizeBaseUrl(options.newApiBaseUrl);
    providers[providerName].apiKey = newApiKey || '';
    providers[providerName].api = 'openai-completions';
    providers[providerName].models = templateProviders[providerName].models;
  }
...
function validateConfig(config, options, newApiKey) {
  ...
  if (options.customer
    && (providers.custom?.apiKey || providers.litellm?.apiKey)) {
