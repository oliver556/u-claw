...
function patchTasksPageEcommerceWorkflow() {
  const helper = [
    ...
    "function UcEcommercePrimaryActionState(e,t,n){let r=Array.isArray(e)?e:[];if(r.length)return{label:`生成图片`,disabled:!0,hint:`还需：${r.join(`、`)}`};if(t)return{label:`任务已创建`,disabled:!0,hint:`任务已创建，待图片接口激活；出一张显示一张。`};let a=Boolean(n&&(Array.isArray(n.images)&&n.images.length||n.error||n.progress||n.completed_at));return a?{label:`重新创建任务`,disabled:!1,hint:`可按当前配置重新创建一个新任务，旧结果会保留在生成记录。`}:{label:`生成图片`,disabled:!1,hint:``}}",
