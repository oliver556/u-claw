...
function patchTasksPageEcommerceWorkflow() {
  const helper = [
    ...
    "function UcEcommerceDefaultForm(){let e=`douyin`;try{e=globalThis.localStorage?.getItem(`uclaw.ecommerceWorkbench.platform.v1`)||e}catch{}let t=UcEcommercePlatformPresets().find(t=>t.id===e)||UcEcommercePlatformPresets()[0];return{platform:e,language:t.defaultLanguage||`zh-CN`,productName:``,category:``,sellingPoints:``,audience:``,outputTypes:[`main_image`,`detail_image`],outputCounts:{main_image:3,detail_image:5,model_image:1},files:[]}}",
    ...
    "function UcEcommerceRecordStatusText(e){return e===`completed`?`已生成`:e===`generating`?`生成中`:e===`failed`?`失败`:e===`draft`?`已记录`:`处理中`}",
  ...
  const redesignedWorkbenchView = [
    ...
    "else if(s.length){let f=s[0];y=i`<section class='uclaw-ecommerce-result'><div class='uclaw-ecommerce-result-head'><div><strong>${u?`正在生成`:o.platform_label+` 已生成图片`}</strong><span>${o.name} · ${s.length}${q?.total?`/${q.total}`:``} 张结果 · ${o.language?.label||S.label} · ${o.model||`默认图片模型`}</span></div><div class='uclaw-ecommerce-result-actions'><button class='btn primary' type='button' ?disabled=${u} @click=${()=>e.downloadEcommercePackage?.()}>打包下载</button><button class='btn' type='button' @click=${()=>e.copyEcommerceManifest?.()}>复制 Manifest</button></div></div>${u&&q?i`<div class='uclaw-ecommerce-progress'><div><span style=${`width:${x}%`}></span></div><small>${q.done||s.length}/${q.total||g} · ${q.current||`继续生成中`} · 出一张显示一张</small></div>`:``}<div class='uclaw-ecommerce-result-body'><figure class='uclaw-ecommerce-featured'><img src=${f.dataUrl||f.url} alt=${f.title||`生成图`}><figcaption><div><strong>${f.title||`生成图`}</strong><span>${f.type||`image`} · ${f.model||o.model||``}</span></div><a class='btn' href=${f.dataUrl||f.url} download=${`01-${UcEcommerceSafeFileName(o.name||`ecommerce`)}-${UcEcommerceSafeFileName(f.title||f.type||`image`)}.${UcEcommerceImageExtension(f)}`} target='_blank' rel='noreferrer'>下载</a></figcaption></figure></div><div class='uclaw-ecommerce-result-strip'><div class='uclaw-ecommerce-generated-grid' aria-label='生成结果横向列表'>${s.map((t,n)=>{let r=`${String(n+1).padStart(2,`0`)}-${UcEcommerceSafeFileName(o.name||`ecommerce`)}-${UcEcommerceSafeFileName(t.title||t.type||`image`)}.${UcEcommerceImageExtension(t)}`;return i`<figure class='uclaw-ecommerce-generated' role='button' tabindex='0' title='点击下载' @click=${()=>e.downloadEcommerceImage?.(t,r)} @keydown=${n=>{(n.key===`Enter`||n.key===` `)&&(n.preventDefault(),e.downloadEcommerceImage?.(t,r))}}><img src=${t.dataUrl||t.url} alt=${t.title||`生成图`}><figcaption><strong>${t.title||`生成图`}</strong><span>${t.type||`image`} · 点击下载</span></figcaption></figure>`})}</div></div>${o.warnings?.length?i`<div class='banner banner-warn'>${o.warnings.join(`；`)}</div>`:``}<div class='uclaw-ecommerce-qa'>${o.qa.map(e=>i`<span>${e}</span>`)}</div></section>`}",
  ...
  const enhancedMethods = methods
    ...
    .replace(
      "this.ecommerceGenerating=!0,this.ecommerceResult=null,this.upsertEcommerceRecord(s);",
      "this.ecommerceGenerating=!0,this.ecommerceActiveRequestId=n.id,this.ecommerceActiveManifest=n,this.ecommerceActiveRecord=s,this.ecommerceProgressStop?.(),this.ecommerceProgressStop=globalThis.uclaw?.onEcommerceImageProgress?.(t=>this.onEcommerceImageProgress?.(t))||null,this.ecommerceResult={...n,provider:`newapi`,model:``,images:[],warnings:[],progress:{done:0,total:u,current:`等待开始`,status:`generating`}},this.upsertEcommerceRecord({...s,result:this.ecommerceResult});",
    )
    .replace(
  ...
  for (const file of listAssetFiles(/^tasks-page-.*\.js$/, "tasks-page")) {
    ...
    after = after.replace(
      ...
      "this.client=null,this.loadGeneration=0,this.ecommerceForm=UcEcommerceDefaultForm(),this.ecommerceResult=null,this.ecommerceGenerating=!1,this.ecommerceRecords=UcEcommerceReadRecords(),this.ecommerceProgressStop=null,this.ecommerceActiveRequestId=null,this.ecommerceActiveManifest=null,this.ecommerceActiveRecord=null,this.ecommercePasteHandler=null}",
    );
