...
function patchConfigModelUsageDashboard() {
    ...
    <div class="uclaw-config-model-dashboard" data-uclaw-config-model-dashboard>
      <section class="uclaw-config-model-summary" aria-label="模型金额与用量">
        <article class="uclaw-config-model-panel uclaw-config-model-balance">
          ...
          <p>\${x.detail}</p>
          <div class="uclaw-config-model-actions"><button class="btn primary" type="button" @click=\${()=>e.onRechargeModelQuota?.()} ?disabled=\${R}>\${R?\`充值中\`:\`充值\`}</button><button class="btn" type="button" disabled>记录</button></div>
        </article>
  ...
  const rechargeModelMethod = `async uclawRechargeModelQuota(){if(this.uclawModelUsage?.recharging)return;this.uclawModelUsage={...this.uclawModelUsageSnapshot(),recharging:!0,rechargeMessage:null,rechargeError:null},this.requestUpdate();try{let e=globalThis.uclaw?.rechargeModelQuota?await globalThis.uclaw.rechargeModelQuota({planCode:\`dev_10\`}):{ok:!1,message:\`充值服务不可用\`};if(!e?.ok)throw Error(e?.message||\`充值失败\`);this.uclawModelUsage={...this.uclawModelUsageSnapshot(),recharging:!1,rechargeMessage:e.message||\`虚拟充值成功\`,rechargeError:null,cloudSummary:e.usage?.status===\`ok\`?e.usage:this.uclawModelUsage?.cloudSummary},this.requestUpdate(),this.uclawReloadModelUsage()}catch(e){this.uclawModelUsage={...this.uclawModelUsageSnapshot(),recharging:!1,rechargeMessage:null,rechargeError:e?.message??String(e)},this.requestUpdate()}}`;
  ...
  for (const file of listAssetFiles(/^config-page-.*\.js$/, "config-page")) {
    ...
    if (!after.includes("uclawRechargeModelQuota(){")) {
      after = after.replace(
    ...
    after = after.replace(
      ...
      "onModelChange:(e,t)=>this.uclawChangeModel(e,t),onRefreshModelUsage:()=>this.uclawReloadModelUsage(),onRechargeModelQuota:()=>this.uclawRechargeModelQuota(),",
    );
    after = after.replace(
    ...
    after = after.replace(
      ...
      "onRefreshModelUsage:()=>this.uclawReloadModelUsage(),onRechargeModelQuota:()=>this.uclawRechargeModelQuota(),setBorderRadius:",
    );
    if (
