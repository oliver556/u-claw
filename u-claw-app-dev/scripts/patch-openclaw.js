...
function patchTasksPageEcommerceWorkflow() {
  ...
  redesignedWorkbenchView = redesignedWorkbenchView
    ...
    .replace(
      ...
      "${t.result?i`<button class='btn' type='button' @click=${()=>e.showEcommerceRecord?.(t)}>查看结果</button>`:``}${t.localDir||t.result?.localDir?i`<button class='btn' type='button' @click=${()=>e.openEcommerceLocalPath?.({path:t.localDir||t.result?.localDir})}>打开文件夹</button>`:``}</div></article>",
    );
