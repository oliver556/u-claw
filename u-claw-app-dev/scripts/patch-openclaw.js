    ...
    <div class="uclaw-config-model-dashboard" data-uclaw-config-model-dashboard>
      <section class="uclaw-config-model-summary" aria-label="模型金额与用量">
        <article class="uclaw-config-model-panel uclaw-config-model-balance">
          <div class="uclaw-config-model-label"><span>账户算力</span><em class=\${x.ok?\`ok\`:\`muted\`}>\${x.label}</em></div>
          <strong>\${v?UcQuickFmtTokens(C):x.ok?x.provider:\`--\`}</strong>
          <p>\${v?\`\${x.detail} · \${UcQuickFmtComputeYuan(C)} · 1 元 = 6kw 算力\`:x.detail}</p>
        ...
        <article class="uclaw-config-model-panel uclaw-config-model-metric"><span>今日消耗</span><strong>\${UcQuickFmtTokens(S)}</strong><small>\${v?\`U-Claw 算力\`:UcQuickFmtCost(u.totalCost)}</small></article>
        <article class="uclaw-config-model-panel uclaw-config-model-metric"><span>近 7 天</span><strong>\${UcQuickFmtTokens(w)}</strong><small>\${v?\`U-Claw 算力\`:UcQuickFmtCost(d.totalCost)}</small></article>
      ...
      <section class="uclaw-config-model-main">
        ...
        <div class="uclaw-config-model-panel uclaw-config-model-ledger">
          ...
          <div class="uclaw-config-model-table"><table><thead><tr><th>时间</th><th>会话</th><th>模型</th><th>Token</th><th>\${v?\`算力\`:\`金额\`}</th></tr></thead><tbody>
            \${v?b.length?b.map(e=>i\`<tr><td>\${UcQuickFmtDate(e.createdAt?e.createdAt*1e3:null)}</td><td>\${e.content||e.requestId||\`New API\`}</td><td>\${UcQuickModelName(e.modelName)}</td><td>\${UcQuickFmtTokens((Number(e.promptTokens)||0)+(Number(e.completionTokens)||0))}</td><td>\${UcQuickFmtCompute(e.quota)}</td></tr>\`):i\`<tr><td colspan="5">暂无 New API 流水。</td></tr>\`:h.length?h.map(e=>{let t=e.usage??UcQuickEmptyTotals(),n=UcQuickSessionModel(e);return i\`<tr><td>\${UcQuickFmtDate(e.updatedAt)}</td><td>\${e.label||e.key||\`未命名\`}</td><td>\${UcQuickModelName(n)}</td><td>\${UcQuickFmtTokens(t.totalTokens)}</td><td>\${UcQuickFmtCost(t.totalCost)}</td></tr>\`}):i\`<tr><td colspan="5">暂无用量流水，发起会话后会在这里出现。</td></tr>\`}
          </tbody></table></div>
        </div>
      </section>
    </div>
