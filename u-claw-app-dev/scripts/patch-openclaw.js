...
function patchTasksPageEcommerceWorkflow() {
  const helper = [
    ...
    "function UcEcommerceDefaultForm(){let e=`douyin`;try{e=globalThis.localStorage?.getItem(`uclaw.ecommerceWorkbench.platform.v1`)||e}catch{}return{platform:e,productName:``,category:``,sellingPoints:``,audience:``,outputTypes:[`main_image`,`detail_image`],files:[]}}",
    ...
    "function UcEcommerceMissing(e){let t=[];return e?.files?.length||t.push(`商品图片`),UcEcommerceSelectedOutputTypes(e).length||t.push(`生成类型`),(e?.productName?.trim()||e?.sellingPoints?.trim())||t.push(`商品名称或卖点`),t}",
    "function UcEcommerceBuildManifest(e){let t=UcEcommerceSelectedPreset(e.platform),n=(e.sellingPoints||``).split(/\\n|，|,|；|;/).map(e=>e.trim()).filter(Boolean).slice(0,5),r=e.files||[],a=UcEcommerceSelectedOutputTypes(e),o=[{type:`main_image`,title:`主图生成目标`,size_rule:t.main,asset_strategy:`参考 hero-image / packshot 模板，以第 1 张商品图为主体识别图，主体占比 60%-80%，留出平台叠加区。`,prompt_brief:`清晰商品主体，干净货架级构图，3 个以内核心信息层级，避免虚构资质和销量。`},{type:`detail_image`,title:`详情图生成目标`,size_rule:t.detail,asset_strategy:`参考 infographic / size-spec 模板，拆成首屏卖点详情：钩子、核心功能、规格/材质、信任证据。`,prompt_brief:`同一 Campaign Style Lock，图文层级可读，证据不足项标为待确认。`},{type:`model_image`,title:`模特图生成目标`,size_rule:`按平台主图/内容流规格优先竖版；服饰可用试穿/隐形模特，配饰/美妆可用真人局部展示。`,asset_strategy:`参考 model-showcase / try-on-virtual / ghost-mannequin 模板，把商品自然放到人物或生活场景里，保留真实外观。`,prompt_brief:`人物自然真实，产品清晰可识别，不编造使用效果、医疗功效或品牌背书。`}];return{id:UcEcommerceId(),name:e.productName?.trim()||`未命名商品`,platform:t.id,platform_label:t.label,source_type:t.source_type,confidence:t.confidence,needs_backend_confirmation:t.needs_backend_confirmation,output_types:a,generated_at:new Date().toISOString(),input:{category:e.category?.trim()||`待补充`,audience:e.audience?.trim()||`默认电商购物人群`,image_count:r.length,selling_points:n,files:r.map(e=>({name:e.name,type:e.type,size:e.size}))},outputs:o.filter(e=>a.includes(e.type)),qa:[...t.qa,`低置信平台需后台规则复核`,`图片生成完成后必须人工审查文字和事实`]}}",
...
openclaw-tasks-page .uclaw-ecommerce-head > div,
...
openclaw-tasks-page .uclaw-ecommerce-type input {
  position: absolute;
  inset: 0;
  opacity: 0;
  pointer-events: none;
...
openclaw-tasks-page .uclaw-ecommerce-type span {
  color: #111827;
  font-weight: 700;
}
