...
function patchConfigModelUsageDashboard() {
    ...
    <div class="uclaw-config-model-dashboard" data-uclaw-config-model-dashboard>
      <section class="uclaw-config-model-summary" aria-label="模型金额与用量">
        <article class="uclaw-config-model-panel uclaw-config-model-balance">
          ...
          <strong>\${v?UcQuickFmtTokens(C):x.ok?x.provider:\`--\`}</strong>
          <p>\${x.detail}</p>
        ...
        <article class="uclaw-config-model-panel uclaw-config-model-metric"><span>今日用量</span><strong>\${UcQuickFmtTokens(S)}</strong><small>\${v?\`New API quota\`:UcQuickFmtCost(u.totalCost)}</small></article>
        <article class="uclaw-config-model-panel uclaw-config-model-metric"><span>近 7 天</span><strong>\${UcQuickFmtTokens(w)}</strong><small>\${v?\`New API quota\`:UcQuickFmtCost(d.totalCost)}</small></article>
        <article class="uclaw-config-model-panel uclaw-config-model-metric"><span>累计流水</span><strong>\${UcQuickFmtTokens(T)}</strong><small>\${E}</small></article>
      </section>
      ...
      <section class="uclaw-config-model-main">
        <div class="uclaw-config-model-panel uclaw-config-model-chart-wrap">
          <div class="uclaw-config-model-head"><div><h2>每日消耗趋势</h2><p>近 14 天 · Tokens</p></div><span>\${v?\`New API\`:\`usage.cost\`}</span></div>
        ...
        <div class="uclaw-config-model-panel uclaw-config-model-ledger">
          ...
          <div class="uclaw-config-model-table"><table><thead><tr><th>时间</th><th>会话</th><th>模型</th><th>Token</th><th>金额</th></tr></thead><tbody>
            \${v?b.length?b.map(e=>i\`<tr><td>\${UcQuickFmtDate(e.createdAt?e.createdAt*1e3:null)}</td><td>\${e.content||e.requestId||\`New API\`}</td><td>\${UcQuickModelName(e.modelName)}</td><td>\${UcQuickFmtTokens((Number(e.promptTokens)||0)+(Number(e.completionTokens)||0))}</td><td>\${UcQuickFmtTokens(e.quota)}</td></tr>\`):i\`<tr><td colspan="5">暂无 New API 流水。</td></tr>\`:h.length?h.map(e=>{let t=e.usage??UcQuickEmptyTotals(),n=UcQuickSessionModel(e);return i\`<tr><td>\${UcQuickFmtDate(e.updatedAt)}</td><td>\${e.label||e.key||\`未命名\`}</td><td>\${UcQuickModelName(n)}</td><td>\${UcQuickFmtTokens(t.totalTokens)}</td><td>\${UcQuickFmtCost(t.totalCost)}</td></tr>\`}):i\`<tr><td colspan="5">暂无用量流水，发起会话后会在这里出现。</td></tr>\`}
          </tbody></table></div>
        </div>
      </section>
    </div>
  ...
  for (const file of listAssetFiles(/^config-page-.*\.js$/, "config-page")) {
    ...
    after = after.replace(
      /async uclawRechargeModelQuota\(\)\{[\s\S]*?this\.requestUpdate\(\)\}\}(?=uclawOpenAdvancedModels\(\))/,
      rechargeModelMethod,
    );
    ...
    after = after.replace(
      ...
      "onModelChange:(e,t)=>this.uclawChangeModel(e,t),onRefreshModelUsage:()=>this.uclawReloadModelUsage(),onOpenRechargeDialog:()=>this.uclawOpenRechargeDialog(),onOpenRechargeRecords:()=>this.uclawOpenRechargeRecords(),",
    );
    after = after.replace(
    ...
    after = after.replace(
      ...
      "onModelChange:(e,t)=>this.uclawChangeModel(e,t),onRefreshModelUsage:()=>this.uclawReloadModelUsage(),onOpenRechargeDialog:()=>this.uclawOpenRechargeDialog(),onOpenRechargeRecords:()=>this.uclawOpenRechargeRecords(),setBorderRadius:",
    );
    if (
