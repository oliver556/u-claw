...
assertIncludes('main.js', main, 'launcher-handoff.json');
...
assertIncludes('main.js', main, 'ensurePortableHomeShellDirs');
assertIncludes('main.js', main, "path.join(portableHome, 'Desktop')");
assertIncludes('main.js', main, "path.join(portableHome, 'Downloads')");
assertIncludes('main.js', main, "path.join(portableHome, 'Documents')");
assertIncludes('main.js', main, "path.join(portableHome, 'Pictures')");
assertIncludes('main.js', main, 'hostDownloads');
assertIncludes('main.js', main, 'UCLAW_HOST_USERPROFILE');
assertIncludes('main.js', main, "downloads || hostDownloads || homeDownloads || fallbackDownloads");
...
assertIncludes('main.js', main, 'sanitizePortableSessionsIndex');
assertIncludes('main.js', main, 'migratePortableSessionArtifacts');
assertIncludes('main.js', main, '[A-Za-z]:[\\\\/]');
assertIncludes('main.js', main, 'AppData[\\\\/]Local[\\\\/]Bavi-box[\\\\/]usb-portable[\\\\/]data-');
assertIncludes('main.js', main, 'session.sessionFile = canonicalSessionFile');
assertIncludes('main.js', main, 'writePortableTrajectoryPointer');
assertIncludes('main.js', main, 'polluted session artifact');
...
assertIncludes('Windows script', win, 'set "HOST_USERPROFILE=%USERPROFILE%"');
assertIncludes('Windows script', win, 'set "UCLAW_HOST_USERPROFILE=%HOST_USERPROFILE%"');
assertIncludes('Windows script', win, 'if not exist "%HOME%\\Desktop" mkdir "%HOME%\\Desktop"');
assertIncludes('Windows script', win, 'if not exist "%HOME%\\Downloads" mkdir "%HOME%\\Downloads"');
assertIncludes('Windows script', win, 'if not exist "%HOME%\\Documents" mkdir "%HOME%\\Documents"');
assertIncludes('Windows script', win, 'if not exist "%HOME%\\Pictures" mkdir "%HOME%\\Pictures"');
assertNotIncludes('Windows script', win, 'start "" "%APP_BIN%"');
assertNotIncludes('Windows script', win, 'start "" /wait "%APP_BIN%"');
assertNotIncludes('Windows script', win, 'Windows desktop app exited with code');
assertIncludes('Windows script', win, ':wait_for_launcher_handoff');
assertIncludes('Windows script', win, 'Windows desktop app is ready; startup terminal will close.');
assertIncludes('Windows script', win, 'Start-Process -FilePath $env:APP_BIN');
assertIncludes('Windows script', win, 'windows-app-launch.env');
assertIncludes('Windows script', win, 'UCLAW_PREPARE_ONLY');
assertIncludes('Windows script', win, "'APP_BIN'");
assertIncludes('Windows script', win, '-join [Environment]::NewLine');
assertIncludes('Windows script', win, 'WriteAllText($env:APP_LAUNCH_ENV_FILE');
assertIncludes('Windows script', win, 'RUN_STATE_FILE');
...
assertIncludes('Windows script', win, 'Stopping {0} old Bavi-box cache process(es)');
assertIncludes('Mac script', mac, '--launch-after "$ROOT/Bavi-box.app"');
assertIncludes('Windows script', win, '--launch-after "%ROOT%\\Bavi-box.exe"');
...
assertIncludes('Windows launcher', winLauncher, 'app');
assertIncludes('Windows launcher', winLauncher, 'scripts');
assertIncludes('Windows launcher', winLauncher, 'Windows-Start-App.bat');
assertIncludes('Windows launcher', winLauncher, 'UCLAW_LAUNCHER_PID=');
assertIncludes('Windows launcher', winLauncher, 'UCLAW_PORTABLE_ROOT=');
assertIncludes('Windows launcher', winLauncher, 'AllocConsole');
assertIncludes('Windows launcher', winLauncher, 'CONIN$');
assertIncludes('Windows launcher', winLauncher, 'CONOUT$');
assertIncludes('Windows launcher', winLauncher, 'launchPreparedApp(root)');
assertIncludes('Windows launcher', winLauncher, 'UCLAW_PREPARE_ONLY=1');
assertIncludes('Windows launcher', winLauncher, 'detachedProcess | createNewProcessGroup');
assertIncludes('Windows launcher', winLauncher, 'syncRootWindowsUpdater(root)');
assertIncludes('Windows launcher', winLauncher, 'cmd.Stdout = io.MultiWriter(consoleOut, logFile)');
assertNotIncludes('Windows launcher', winLauncher, 'io.MultiWriter(os.Stdout, logFile)');
assertNotIncludes('Windows launcher', winLauncher, 'HideWindow: true');
assertNotIncludes('main.js', main, 'requestAppQuit({ confirm: false, exitCode: ACTIVATION_RESTART_EXIT_CODE, showShutdownPage: false })');
assertIncludes('main.js', main, 'restartRequired: false');
assertIncludes('main.js', main, 'launchReady: true');
assertIncludes('activation.html', activationHtml, 'if (launchReady) await launchMain();');
...
assertIncludes('package-portable.js', packagePortable, "path.join(stageRoot, 'Bavi-box.app')");
assertIncludes('package-portable.js', packagePortable, "path.join(stageRoot, 'Bavi-box.exe')");
assertIncludes('package-portable.js', packagePortable, "path.join(stageRoot, 'app', 'scripts')");
assertIncludes('package-portable.js', packagePortable, "'app/scripts/Mac-Start-App.command'");
assertIncludes('package-portable.js', packagePortable, "'app/scripts/Windows-Start-App.bat'");
assertIncludes('package-portable.js', packagePortable, "'app/scripts/Windows-Sync-Data.ps1'");
assertIncludes('package-portable.js', packagePortable, "'Bavi-box.exe'");
assertIncludes('package-portable.js', packagePortable, "'Bavi-box.app'");
