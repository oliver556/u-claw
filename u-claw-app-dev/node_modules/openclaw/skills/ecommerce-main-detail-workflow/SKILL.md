...
这是 Bavi-box 的 Workbench-first 能力。前台不暴露聊天、不要求用户理解 OpenClaw session；用户只需要选择平台、上传商品图片、填写少量商品信息，再获得平台化输出预案、Prompt Pack 与 QA 清单。

首版边界：不绕过现有图片模型配置、不读取独立 API key、不替代人工合规审查。图片生成链路未确认前，前台只能输出可复核的 manifest、规格预案和 Prompt Pack，不得伪造成品图。
...
8. Human Review Gate：提示用户确认事实、合规和视觉一致性，再进入可选出图或交付。
