...
description: "Bavi-box Workbench-first capability for ecommerce main images and detail images, covering platform preset, intake, compliance boundary, Campaign Style Lock, storyboard, prompt pack, and QA."
metadata:
  {
    "openclaw":
      {
...
这是 Bavi-box 的 Workbench-first 能力。前台不要求用户理解 OpenClaw session；用户只需要选择平台、上传商品图片、填写少量商品信息，再由工作台创建生成会话、投递图片生成任务并保留记录。

当前边界：不绕过现有图片模型配置、不读取独立 API key、不替代人工合规审查。前台可以投递带图片附件的生成请求，但必须把生成结果视为待审素材，不能承诺一次生成即可合规上架。

## 适用场景

当用户要为电商商品制作主图、详情页长图、平台首图、卖点图、对比图、场景图或整套商品视觉方案时使用本技能。
...
5. Main Image Storyboard：输出 5 张主图方案，每张必须有独立任务、画面结构、文案层级、证据点和风险备注。
6. Detail Page Storyboard：输出 7-9 屏详情图叙事，覆盖吸引、解释、证明、对比、使用场景、信任和行动。
7. Prompt Pack：为每张图输出生成 prompt、negative prompt、尺寸、构图、文字建议、素材要求和 QA checklist。
8. Image Generation：通过 OpenClaw 既有聊天/工具链调用图片生成能力，优先生成 1 张主图和 1 组详情图方向图。
9. Human Review Gate：提示用户确认事实、合规和视觉一致性，再进入下载、重试或交付。
...
- 图片生成记录：平台、商品、素材数量、生成会话、runId、状态和错误信息。
